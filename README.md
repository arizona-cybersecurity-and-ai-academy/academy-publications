...
Arizona Sites (Quickstart) cannot load modules, so the page is maintained by hand from this output. The editor accepts Markdown and nothing more technical (Veronika Kyles, 2026-09-10), so the files to paste are `docs/publications.md`, `docs/grants.md` and `docs/workshops.md` (the group's "Workshops and Presentations" collection); the site applies its own theme. The HTML outputs are kept for any future embed option.
